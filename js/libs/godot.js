...
var Godot = (() => {
	...
	return async function (moduleArg = {}) {
		...
		(function () {
			...
			var currentSafariVersion =
				...
				navigator.userAgent.match(/Version\/(\d+\.?\d*\.?\d*)/)
					? humanReadableVersionToPacked(navigator.userAgent.match(/Version\/(\d+\.?\d*\.?\d*)/)[1])
					: TARGET_NOT_SUPPORTED;
			...
			var currentFirefoxVersion =
				typeof navigator !== "undefined" && navigator?.userAgent?.match(/Firefox\/(\d+(?:\.\d+)?)/)
			...
			var currentChromeVersion =
				typeof navigator !== "undefined" && navigator?.userAgent?.match(/Chrome\/(\d+(?:\.\d+)?)/)
		...
		function findWasmBinary() {
			return locateFile("godot.web.template_release.wasm32.nothreads.minigame.wasm");
		}
		...
		var GodotAudio = {
					...
					positionWorker.onMessage((event) => {
						if (event.type === "position") {
							this._playbackPosition =
								parseInt(event.data, 10) / this.getSample().sampleRate + this.offset;
						}
					});
			...
			start_sample: function (playbackObjectId, streamObjectId, busIndex, startOptions) {
				...
				GodotAudio.SampleNode.create({ busIndex, id: playbackObjectId, streamObjectId }, startOptions);
			},
		...
		function assignWasmExports(wasmExports) {
			...
			__Z14godot_web_mainiPPc = Module["__Z14godot_web_mainiPPc"] = createExportWrapper(
